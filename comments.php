...
<?php if ( have_comments() ) : ?>
	<h3 id="comments"><?php comments_number('沙发犹在', '板凳尚存', '大战 % 回合' );?> to &#8220;<?php the_title(); ?>&#8221;</h3>
 ...
 <?php else : // this is displayed if there are no comments so far ?>
	...
	<?php if ( comments_open() ) : ?>
	 ...
	 <?php else : // comments are closed ?>
		...
		<p class="nocomments">评论太黄,被绿爸干了</p>
...
<label for="author"><small> 江湖人称 <?php if ($req) echo "(required)"; ?></small></label></p>

<p><input type="text" name="email" id="email" value="<?php echo esc_attr($comment_author_email); ?>" size="22" tabindex="2" <?php if ($req) echo "aria-required='true'"; ?> />
<label for="email"><small> 君之信址(隐) <?php if ($req) echo "(required)"; ?></small></label></p>

<p><input type="text" name="url" id="url" value="<?php echo esc_attr($comment_author_url); ?>" size="22" tabindex="3" />
<label for="url"><small> ブログ </small></label></p>
